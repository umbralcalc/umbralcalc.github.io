...
where $K(z,z';H)$ is some smoothing kernel which helps to approximate the posterior distribution up to some specified scale using the bandwidth matrix $H$. Note that the expression above can also be extended to make use of the full history of $z$ samples by applying the discount factor $\beta$ as discussed in the previous section. For advanced readers interested in configuring the stochadex to use this SIR technique, an implementation can be found within this repository: [https://github.com/umbralcalc/dexetera](https://github.com/umbralcalc/dexetera).
