---
title: "Backtesting and other challenges"
tag: "Trusting Actions on Simulated Systems"
series-blurb: "A collection of posts on how we decide whether an action chosen with a simulation can be trusted. Written especially for programmers and non-technical readers wanting to learn about how to take better actions under uncertainty. No maths; just diagrams and straightforward descriptions all the way through."
order: 4
---

# Backtesting and other challenges
<div style="height:0.75em;"></div>

## Backtesting actions-to-outcomes

Cover these topics:
- Lack of data on this generally because its hard to control for cause-to-effect
- Quasi-experiments all kinds and the action2outcome project

## The one-history problem

Cover:
- Relate back to single trajectory discussion in previous post
- All ways around this make assumptions which might be broken

## Concept drift is another problem

Cover:
- Concept drift and how it affects this
- Adaptive simulations


